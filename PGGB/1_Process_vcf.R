...
# Save combined VCF information for all chromosomes = input for gene intersection
vcf_clean_func <- vcf_clean
vcf_clean_func$REF_accn <- ifelse(vcf_clean$REF_accn == "","NA",vcf_clean$REF_accn)
vcf_clean_func$ALT_accn <- ifelse(vcf_clean$ALT_accn == "","NA",vcf_clean$ALT_accn)
vcf_clean_func <- vcf_clean_func %>% select(-N_allele, -N_bp_ref, -N_bp_diff, -singletons, -missing, -bi_multi, -par_not)
fwrite(vcf_clean_func,"whole_genome/VCF_CLEAN_allchrs.txt",sep = "\t",quote = F,row.names = F,col.names = F)

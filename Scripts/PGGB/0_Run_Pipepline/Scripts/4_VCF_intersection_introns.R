...
vcf_intersect_nr <- vcf_intersection_indels %>%
  ...
  ungroup()
# for questioning mRNA and gene to introns
vcf_intersect_nr <- data.table(vcf_intersect_nr) %>%
  mutate(gene_struc = ifelse(gene_struc %in% c("mRNA", "gene"), "Intron", gene_struc))
...
plot_gene_struc(vcf_intersect_raw_stats_INDEL,N_bubble, x_label="Subtype", y="Number of Occurences")
plot_gene_struc(vcf_intersect_raw_stats_SV,N_bubble, x_label="Subtype", y="Number of Occurences")
...
plot_Length(vcf_intersect_raw_stats_INDEL, Variant_length, x_label="Gene Structure","Cumulative length (in Kb)", 1e3)
...
by(vcf_intersect_raw_stats$Average_length, vcf_intersect_raw_stats$gene_struc, shapiro.test)
by(vcf_intersect_raw_stats_INDEL$N_bubble, vcf_intersect_raw_stats_INDEL$gene_struc, chisq.test)
